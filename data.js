// Sample data structure for team responses
const teamData = {
    questions: [
        "Have you ever ridden a horse?",
        "Have you ever fallen in love?",
        "Have you ever been to a soccer game?",
        "Have you ever been to the beach?",
        "Have you ever traveled to another country?"
    ],
    members: [
        {
            id: 1,
            name: "Nissy Dayana Oliva Amezquita",
            responses: [
                "Yeah, I’ve ridden a horse three times. The first two were on my boyfriend's horse, he taught me the basics of how to get on and steer. I actually rode bareback, which was a bit risky since the horse was still getting used to being ridden, but it was totally worth it",
                "Yes, right now with my boyfriend! We’ve been together for years. Falling in love is really nice, but you definitely have to learn how to mature together to make the relationship grow.",
                "No, I’ve never been to a stadium before. I just haven’t had the chance yet, so I've only watched games on TV.",
                "Not yet, but I’d love to go someday",
                " I haven’t had the chance to travel abroad yet, but I really hope to do it in the future."
            ]
        },
        {
            id: 2,
            name: "Johan Sebastian Puentes Pulido",
            responses: [
                "No, i have not had the opportunity to ride a horse",
                "Yes, i have fallen in love many times.",
                "No, i have't been to a soccer game but i'd like to",
                "Yes, i have gone to lorica's beach, a beautiful city with awesome landscapes and clean water from the Sinú river",
                "No, i have never traveled to another country, but i'd like to visit Russia"
            ]
        },
        {
            id: 3,
            name: "Dylan Alejandro Guzman Gomez",
            responses: [
                "Yes, I went horseback riding on vacation last year.",
                "Not yet, but I'm hoping to find love soon.",
                "Yes, I played soccer in college.",
                "Yes, I spent my whole childhood at the beach."
            ]
        },
        {
            id: 4,
            name: "Cristian Danilo Valle Amortegui",
            responses: [
                "No, I'm afraid of horses.",
                "Yes, I've been in love since high school.",
                "No, I prefer watching tennis.",
                "No, I live inland but visit beaches when traveling."
            ]
        },
        {
            id: 5,
            name: "Dominik Alejandro Cely Diaz",
            responses: [
                "Yes, I've ridden a horse",
                "No, I've never fallen in love",
                "No, I have never been to a soccer game",
                "Yes, I've been to the beach many times",
                "No, i have not traveled to another country but i'd like to visit panama"
            ]
        },
    ]
};

// Make the data available globally
window.teamData = teamData;
